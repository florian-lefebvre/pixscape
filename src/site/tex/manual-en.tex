...
For vector data, Pixscape supports 2 file formats : geopackage (.gpkg) by default or shapefile (.shp)
...
The Visibility / Multi Viewshed menu allows you to calculate several viewsheds from a point layer containing observation points.
...
\begin{itemize}
	\item Point layer : layer containing observation points.
	\item Id : layer attribute used as identifier. This parameter is useful only for vectorial result.
	\item Inverse: unchecked by default, the points layer represent the observation points \textit{ie.} the eye of the observer. Conversely, if the box is checked, the points layer becomes the observed points. For details see the viewshed \nameref{viewshed_param}.
	\item Dest. height (h2): height in meters of observed point(s). If set to -1, the height of the DSM is used. If the project does not contain a DSM, the height is zero.
	\item Bounds: this window allows you to restrict the eyesight of the observer in the 3 dimensions for all observation points (see \nameref{bounds}). If the layer contains the attributes (dmin, dmax, zmin, zmax, orien et amp), they will be used in place of these settings.
...
These bounds can be set from the Bounds window (figure \ref{bounds_dlg}). When the observation points come from a point layer, the setting of the eyesight can be differentiated for each point by adding the 6 parameters (dmin, dmax, zmin, zmax, orien, amp) in the layer attributes table. The "Set point attributes" menu allows you to automatically add them to an existing layer (see \nameref{add_attributes}).
...
\begin{itemize}
	\item Inverse (planimetric only): unchecked by default, the points layer represent the observation points \textit{ie.} the eye of the observer. Conversely, if the box is checked, the points layer becomes the observed points. For details see the viewshed \nameref{viewshed_param}.
	...
	\item Bounds: this window allows you to restrict the eyesight of the observer in the 3 dimensions for all observation points (see \nameref{bounds}). If the selected sampling is Points and the layer contains the attributes (dmin, dmax, zmin, zmax, orien et amp), they will be used in place of these settings.	
\end{itemize}
...
Instead of using the raster grid sampling, you can provide a set of points from a point layer.

The result will be in the form of a point layer corresponding to the input layer. The result of the metrics will be stored in the attribute table of the layer.
...
The layer "Metric point result" can be exported in geopackage, shapefile or text format by right clicking on the layers name and the menu "Export ...".
...
The Tools / Set point attributes menu is useful for specifying different restrictions for each observation point. This function adds the 6 attributes to a layer with the default values defined by the Bounds window. You can then with your favorite GIS modify the restriction values for each observation point.
The output layer can be used for calculation of metrics (\ref{calc_metrics}) or multi viewshed (\ref{multi_viewshed}).

\begin{figure}[H]
	\includegraphics[scale=0.5]{img/add_attributes-en.png} 
	\caption{Add eyesight bound attributes to a layer}
...
\begin{itemize}
	\item \verb|prj_name| : name of the new project
	\item \verb|dtm_raster_file| : DTM raster layer
\end{itemize}

\subsubsection{Optional parameters}
\begin{itemize}
	\item \verb|dsm=raster_file| : 
	\item \verb|landuse=raster_file| : 
	\item \verb|dir=path| : 
\end{itemize}
...
\subsection{--addscale : add scale data}
\begin{Verbatim}[commandchars=\\\{\}]
...
\begin{Verbatim}[commandchars=\\\{\}]
--landmod zone=\textit{filezones.gpkg} id=\textit{fieldname} code=\textit{fieldname} 
height=\textit{fieldname} | dsm=\textit{file.tif}
[selid=\textit{id1,...,idn}]
\end{Verbatim}

\subsubsection{Required parameters}
\begin{itemize}
	\item \verb|zone=filezones.gpkg| : polygon layer containing land use changes.
	\item \verb|id=fieldname| : field name of the layer used for identifying polygons. If values are not unique, polygons with the same identifier will be applied in a single change.
	\item \verb|code=fieldname| : field name of the layer storing the new land use category of the polygon.
	\item \verb+height=fieldname | dsm=file.tif+ : the new values of the DSM used for each land use change may be given from a field of the polygon layer (\verb|height| parameter) or from a raster (\verb|dsm| parameter). In the latter case, the geometry of the raster must be exactly the same as the project DEM.
\end{itemize}

\subsubsection{Optional parameter}
\begin{itemize}
	\item \verb|selid=id1,...,idn| : list of polygon identifiers to process. If this parameter is not set, all polygons of the layer are processed.
\end{itemize}

\subsubsection{Description}
This command must be placed before the visibility calculation commands. It will duplicate the project for each polygon of the layer and change the landuse covered by the polygon and the DSM. The commands following this one, will be executed on each modified project.
...
\subsubsection{Optional parameters}
\begin{itemize}
	\item \verb|point| : coordinates of the observation point in the coordinate system of the DTM. If this parameter is not set, the observation points will correspond to the sampling layer from the \verb|-sampling| parameter.
...
\subsubsection{Description}
The \verb|--viewtan| command allows to calculate the tangential view from an observation point. Results are stored in 2 or 3 raster files in Tiff format depending on whether the project contains a land use layer, and 
ainsi qu'un fichier au format png pour un rendu visuel plus claire reprenant les codes couleurs des classes d'occupation du sol.
\begin{itemize}
	\item \verb|elev| : elevation of the DTM
	\item \verb|dist| : 2D distance from the observation point
	\item \verb|land| : land use, if present in the project, in tiff and png formats
\end{itemize}
...
\subsubsection{Examples}
The example below calculates the tangential view for an observer located at the point (925560, 6702495). The result is stored in the projet folder in a files named: \verb|viewtan-925560,6702495-elev.tif|, \verb|viewtan-925560,6702495-dist.tif|, \verb|viewtan-925560,6702495-land.tif| and \verb|viewtan-925560,6702495-land.png| if the project contains a land use layer.
\begin{Verbatim}
...
\begin{Verbatim}
	--viewtan prec=0.05 point=925560,6702495 resname=view1
\end{Verbatim}
...
\begin{itemize}
	\item \verb/format=vector|raster/ : result format (geopackage or tif)
\end{itemize}
...
The result is stored in a raster file or a vector layer depending on the selected format.
...
In vector, the result is a geopackage layer storing the viewshed of each observation point.

\subsubsection{Example}
The example below calculates the viewsheds of the observation points from the layer points.gpkg and stores the result in a geopackage file in the project folder named: \verb|multiviewshed.gpkg|.
\begin{Verbatim}
...
Results are stored in several rasters for a grid sampling and in one layer for a vector sampling.
...
\subsubsection{Required parameters}
\begin{itemize}
	\item \verb|objects=pointfile.gpkg| : layer of observed points
	\item \verb|id=fieldname| : layer field containing an identifier
\end{itemize}

\subsubsection{Optional parameters}
\begin{itemize}
	\item \verb/degree=height|area/ : angular height or surface result
...
Without verb|agreg| parameter, the result contains the list of points couples (observer, observed) where observer sees the observed point. If the \verb|degree| option is used, the column \verb|View| of the result file contains the angular height or surface.\\
With \verb|agreg| parameter, the result is aggregated by observer or observed point and the column \verb|View| contains the sum of the grouped values.

\subsubsection{Example}
This example calculates the intervisibility of the points given in points.gpkg and stores the result in the file \verb|toobject.csv| in project directory. In this example, observation and observed points are the same.
\begin{Verbatim}
...
\end{Verbatim}
Point sampling needs a layer of points with an identifier field.
The result of \verb|--planmetric| and \verb|--tanmetric| commands is stored in a layer named \verb|metrics-pointfile.gpkg|.
...
The example above loads the project \verb|Project1/Project1.xml| and calculates several tangential metrics (\verb|--tanmetric A SL DL|) from a set of observation points defined in the layer \verb|Points_IDF.gpkg|. The result will be saved in a layer named \verb|metrics-Points_IDF.gpkg|, containing the observation points, and the values of the 3 metrics.
