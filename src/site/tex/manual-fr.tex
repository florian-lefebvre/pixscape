...
La table attributaire de la couche résultat est accessible par un clic droit sur le nom de la couche et le menu "Table attributaire". Elle contient toujours, en plus des métriques, les 6 paramètres de limitation du champs de vision, permettant de conserver l'information de paramétrage avec les résultats.
...
\end{Verbatim}
L'option \verb|-bounds| permet de restreindre le champs de vision dans les 3 dimensions par 6 paramètres : \verb|dmin|, \verb|dmax|, \verb|zmin|, \verb|zmax|, \verb|orien|, \verb|amp|. Pour plus de détails sur les paramètres, voir \nameref{bounds}.
...
La hauteur des pixels correspond à l'altitude du MNT plus la hauteur du MNE si il est présent. La hauteur du point d'observation O correspond à l'altitude du MNT plus le paramètre h1 (Hauteur de l'oeil de l'observateur). La hauteur du MNE n'est jamais prise en compte pour la hauteur de l'observateur (l'observateur ne grimpe pas aux arbres).
